...
export const CheckoutForm = ({ total, products }) => {
  ...
  useEffect(() => {
    ...
    total &&
      trackEvent("InitiateCheckout", {
        content_ids: products.map((product) => product._id), // Asumiendo que 'cart.products' es un array de productos
        content_type: "product",
        value: total, // Suma total del carrito
        currency: "ARS",
      });
  }, [total]);
  ...
  const sendMessage = () => {
    trackEvent("Purchase", {
      content_ids: products.map((product) => product._id), // Suponiendo que esto representa el ID del producto comprado
      content_type: "productos", // El título del producto
      value: total, 
      currency: "ARS",
    });
